...
- 涉及 UI 的 round 必须填写任务卡「视觉审查」字段。用户在截图请求发出后 30 分钟内未完整回传，或明确拒绝截图时，可用 `CODE_ONLY` 兜底；通过后固定记录 `CODE_ONLY_PASS`、`截图验证：未提供（SCREENSHOT_NOT_PROVIDED）` 及 `TIMEOUT_30M | USER_DECLINED`。该标记不等于真实截图验证，但不阻塞后续 PR 流程。
