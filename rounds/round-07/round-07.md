...
- 首轮人工验收发现 5 项阻断：长会话全量渲染掉帧、工具卡始终展开、右目录不能隐藏、目录点击不能可靠定位、“跟随最新”无响应。已改为 GPUI variable-height `list` + 持久 `ListState`，只构建视口附近消息；工具 input/output/diff/read 正文默认折叠且展开前不进入 element tree；目录支持隐藏/恢复并通过 `ListOffset` 逻辑消息索引跳转；跟随按钮直接 `scroll_to_end()` 后重新进入 Tail 模式。修复后 `./scripts/validate.sh` 再次 `VALIDATE OK`，日志：`.pi/round-07-validate-acceptance-fixes.log`。随后人工复验发现消息区被父 flex 布局压成 0 高度；已给 message list wrapper 补 `h_full()`，并恢复自动化断言：消息 viewport 高度必须大于 0、`chat-message` 必须实际进入渲染树。进一步按 pi-web 对齐 turn 展示：目录仅保留用户 Query / 最终 Assistant Answer；完成 turn 的中间 trace 汇总为默认折叠的“处理详情”，活跃未完成 turn 保持展开；thinking 与工具卡均二级折叠，点击后才创建详情正文并局部重新测量虚拟列表项。对应全量验证再次通过：`.pi/round-07-validate-turn-folding.log` → `VALIDATE OK`；DeepSeek V4 Pro 独立复审最终结论为无 Blocker/High。
- T3 长回复首轮人工结论为“不通过”；完成虚拟列表、目录/跟随修复、turn 过程折叠及 thinking/tool 二级惰性展开后重新验收。用户于 2026-08-17 明确确认“验收通过”：长会话正文正常、交互流畅，右侧目录仅展示 Query / final Answer，完成 turn 的处理详情默认折叠，thinking 与工具详情可按需展开。
